 ...
 * 組織架構圖主要元件
 * 
 * 這個元件提供了一個完整的組織架構圖解決方案，包含以下功能：
 * - 互動式組織架構圖，支援拖拽、縮放、平移操作
 * - 智能自動排版系統，採用樹狀布局算法
 * - 拖拽操作：支援節點間的「替換」和「交換」資料功能
 * - 員工資料管理：新增、編輯、刪除員工資料
 * - 匯入/匯出：支援 JSON 格式的資料匯入匯出
 * - 主題切換：支援深淺色主題
 * - 視圖控制：縮放、適應視窗、格線顯示
 * - 小地圖導航和即時統計資訊
 * 
 ...
 * 
 * 拖拽功能說明：
 * - 替換操作：目標節點獲得來源節點資料，來源節點被清空
 * - 交換操作：兩個節點完全交換彼此的資料
 * - 支援拖拽操作選擇彈窗，讓使用者決定操作類型
 * 
 * 自動排版特色：
 * - 智能計算子樹寬度，避免節點重疊
 * - 支援多根節點的布局處理
 * - 父節點自動居中於子節點上方
 * - 使用 smoothstep 連線類型，產生直角轉折效果
 * 
 * @author Claude Code
 * @version 1.0.0
 */
